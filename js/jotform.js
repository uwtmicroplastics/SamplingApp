...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    initMultipleUploads: function () {
        ...
        // but not in jotform next. fix for #1816394
        if (JotForm.browserIs.mobile() && !JotForm.isJotFormNext) {
    ...
    countTotal: function (prices) {
        ...
        $H(prices).each(function (pair) {
            ...
            if ($(pair.value.specialPriceField)) {
                ...
                if (pair.value.child && pair.value.specialPriceField.split("_").length === 4) {
                    ...
                    var item = null;
                    if ($(pair.value.quantityField) && $(pair.value.quantityField).closest("tr")) {
                        item = $(pair.value.quantityField).closest("tr").querySelector("th").textContent.trim();
                    } else {
                    ...
                    if ($(pair.value.quantityField) && (pair.value.specialPriceField !== pair.value.quantityField)) {
                        ...
                        if (document.querySelector('#' + parentProductKey + '_subproducts')) {
                            specialName.push({
                                ...
                                value: $(pair.value.quantityField).closest("tr").querySelector("th").textContent.trim()
                            });
                        }
                    }
        ...
        if (JotForm.payment === "paypalSPB") { // This condition added for not broken another gateways. It's only for preventation. Next, we can delete it if we need for another gateways.
        ...
        if (window.signatureForm) {
            ...
            for (var i = 0; i < pads.length; i++) {
                ...
                if (jQuery(pad).attr("data-required") === "true") {
                    ...
                    if (formLine.is(':visible') && jQuery(pad).jSignature('getData', 'base30')[1].length == 0 && !jQuery(pad).hasClass('edit-signature')) {
                        ret = false;
